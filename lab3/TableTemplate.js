...
class TableTemplate {
    static fillIn(id, dict, columnName) {
        ...
        let cols = [];
        if (columnName === undefined) cols = Array.from(Array(header.cells.length).keys())
        else for (let i = 0; i < header.cells.length; ++ i) 
                if (header.cells[i].innerHTML === columnName) cols = [i];
